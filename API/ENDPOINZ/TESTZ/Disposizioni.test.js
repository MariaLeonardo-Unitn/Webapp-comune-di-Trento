...
describe('test disp methods', () => {
    ...
    test('POST calendario', () => {
        return request(app).post('/api/rifiuti/disposizioni/')
        .set('access-token', token)
        .set('Accept', 'application/pdf')
        .attach('file', fakePdf, 'disposizione.pdf')
        .expect(201)
    });

    //TEST GET /calendari/zona
    test('GET calendario', () => {
        return request(app).get('/api/rifiuti/disposizioni/')
        .set('access-token', token)
        .set('Accept', 'application/pdf')
        .expect(200)
    });
    ...
    test('PATCH calendario', () => {
        ...
        return request(app).patch('/api/rifiuti/disposizioni/')
        .set('access-token', token)
        .set('Accept', 'application/pdf')
        .attach('file', newFakePdf, 'disposizione.pdf')
        .expect(200);
    })

    //TEST DELETE /calendari/zona
    test('DELETE calendario', () => {
        return request(app).delete('/api/rifiuti/disposizioni/')
        .set('access-token', token)
        .expect(200);
    });
});